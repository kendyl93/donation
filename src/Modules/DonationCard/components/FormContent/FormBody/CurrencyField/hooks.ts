...
export const useCurrencyField = () => {
  ...
  const handleAmountOnFocus = (event: ChangeEvent<HTMLInputElement>) =>
    formAmountWithoutFormatting(event?.target?.value);
  ...
  const preventNonNumericInput = (
    ...
    const maybeReachedMaximum =
      isNumber(amount) && amount?.toString()?.length > MAX_VALUE_LENGTH;
    const isKeyPressedNumeric = regex.numeric.test(keyPressed);

    if (
      maybeReachedMaximum ||
      !isKeyPressedNumeric ||
      (keyPressed === "0" && !event.currentTarget.value)
    ) {
